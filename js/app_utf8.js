...
// app.js に追加するロジックのプレビュー
async function updateCacheListUI() {
    const listContainer = document.getElementById('dev-cache-list');
    const keys = await getAllAudioKeys();

    listContainer.innerHTML = keys.map(key => `
        <div style="display: flex; align-items: center; justify-content: space-between; border-bottom: 1px solid rgba(255,255,255,0.1); padding: 4px 4px; gap: 10px;">
            <span style="flex: 1; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; color: #e2e8f0; font-size: 11px;" title="${key}">${key}</span>
            <div style="display: flex; gap: 8px; flex-shrink: 0;">
                <button onclick="testPlayCache('${key}')" 
                        style="background: #334155; border: none; color: #94a3b8; width: 28px; height: 28px; border-radius: 4px; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: all 0.2s;"
                        onmouseover="this.style.background='#475569'; this.style.color='#fff';" 
                        onmouseout="this.style.background='#334155'; this.style.color='#94a3b8';">▶</button>
                <button onclick="deleteCacheItem('${key}')" 
                        style="background: #450a0a; border: none; color: #ef4444; width: 28px; height: 28px; border-radius: 4px; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: all 0.2s;"
                        onmouseover="this.style.background='#7f1d1d'; this.style.color='#fca5a5';" 
                        onmouseout="this.style.background='#450a0a'; this.style.color='#ef4444';">✕</button>
            </div>
        </div>
    `).join('') || '<div style="text-align:center; padding: 20px; color: #64748b;">キャッシュは空です</div>';
}

// 全てのキャッシュキーを取得する
async function getAllAudioKeys() {
...
async function deleteCacheItem(key) {
    if (!confirm(`キャッシュ「${key}」を削除しますか？`)) return;
    // STORE_NAME を使う
...
// キャッシュを再生テストする
async function testPlayCache(key) {
...
// グローバルスコープに出さないと HTML の onclick から呼べないため
window.testPlayCache = testPlayCache;
window.deleteCacheItem = deleteCacheItem;
...
async function init() {
    ...
    // TTSテストボタンの処理を追加
    const ttsTestBtn = document.getElementById('dev-btn-test-tts');
    const ttsTestInput = document.getElementById('dev-tts-input');
    ...
    if (ttsTestBtn) {
        ttsTestBtn.onclick = async () => {
            ...
            // getAudioBlob をテスト (type='orig'、rubyなし)
            // キャッシュを無視して最新の読みをテストしたい場合はキーを変える必要がありますが
            // まずは普通に呼び出します。
            const blob = await getAudioBlob(text, 'orig', text.split(''));
            if (blob) {
                playBlob(blob);
                console.log("TTS Test playing:", text);
            }
        };
    }
    ...
    // キャッシュ更新ボタン
    const refreshCacheBtn = document.getElementById('dev-btn-refresh-cache');
    if (refreshCacheBtn) {
        refreshCacheBtn.onclick = updateCacheListUI;
    }
 ...
 */
async function getAudioBlob(word, type = 'orig', ruby = null, phonetic = null) {
    ...
    try {
        let textToSpeak;
        if (phonetic) {
            // ★データベースの「完成形」をそのまま使用（最速）
            textToSpeak = phonetic;
        } else if (type === "rev" || type === "orig") {
            // バックアップ：phoneticがない場合のみ計算する
            const baseText = Array.isArray(ruby) ? ruby.join("") : word;
            textToSpeak = hiraganaToKatakana(baseText) + "、";
        } else {
        ...
        // 念押しでカタカナ化（phoneticがすでにカタカナなら何もしない）
...
async function preloadAudios(questions) {
    ...
    for (let i = 0; i < total; i++) {
        ...
        for (let char of q.ruby) {
            await getAudioBlob(char, 'parts', [char]);
        }

        // 2. Preload the FULL word (reversed) for "Master Answer"
        await getAudioBlob(q.reverse, 'rev', q.reverse.split(''), hiraganaToKatakana(q.reverse) + '、');
...
async function readSequenceNatural(word) {
    try {
        //currentState.currentQuestion.phonetic を渡す
...
function submitAnswer(rawAnswer) {
    ...
    saveLog({
        ...
        original: currentState.originalWord,
...
async function playMasterAudio() {
    ...
    const blobs = [];
    for (let unit of sequence) {
        blobs.push(await getAudioBlob(unit, 'parts', [unit]));
    }
...
async function initDevMode() {
    ...
    updateCacheListUI(); // デベロッパー画面を開いた時にリストを表示
    switchDevTab('moras');
}
